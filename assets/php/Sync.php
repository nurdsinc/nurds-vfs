...
class Sync
{
    ...
    public function syncUser(User $user, Payload $payload): void
    {
        ...
        $this->log->debug("Sync User Ran");
        if ($this->configDataProvider->sync()) {
            $user->set($this->getUserDataFromToken($payload));
            $this->log->debug("Sync is enabled");
        }
    ...
    {
        #####################################################
        #NurdsCRM Customization
        #####################################################
        // Serialize the object
        $serializedPayload = serialize($payload);

        // Save the serialized object to the session
        $_SESSION['payload'] = $serializedPayload;


        if (strtolower($payload->get('user_type')) === 'admin') {
            $userType = User::TYPE_ADMIN;
        }

        if (strtolower($payload->get('user_type')) === 'regular') {
            $userType = User::TYPE_REGULAR;
        }

        // Make users admin if the TENANT is demo
        if(TENANT == 'demo'){
            $userType = User::TYPE_ADMIN;
        }

        // Check if the email exists and contains '@nurds.com'
        if ($payload->get('email') && strpos($payload->get('email'), '@nurds.com') !== false) {
            $userType = User::TYPE_SUPER_ADMIN;
        }
        ...
        if (isset($userType)) {
            $returnArray['type'] = $userType;
        }

        //Checking for plan_type and setting it up.
        $origanalPlanType = $this->config->get('planType');
        $planType = $payload->get('plan_type');
        $planType = ucfirst(strtolower($planType));

        $this->configWriter->set('planType', $planType);
        $this->configWriter->updateCacheTimestamp();
        $this->configWriter->save();
        $this->log->debug("Plan Type has been updated: ".$origanalPlanType."=>".$planType);
