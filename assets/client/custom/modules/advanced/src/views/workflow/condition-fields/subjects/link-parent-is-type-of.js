...
['view', 'advanced:workflow-helper'], function (Dep, Helper) {
    ...
    return Dep.extend({
        ...
        setup: function () {
            ...
            let foreignEntityTypeList = this.getMetadata()
            ...
            this.getModelFactory().create(entityType, (model) => {
                ...
                if (!foreignEntityTypeList.length) {
                    foreignEntityTypeList = Object.keys(this.getMetadata().get(['scopes'])).filter((item) => {
                        return !!this.getMetadata().get(['scopes', item, 'object']);
                    });

                    foreignEntityTypeList.push('CampaignTrackingUrl');

                    foreignEntityTypeList = foreignEntityTypeList.sort((v1, v2) => {
