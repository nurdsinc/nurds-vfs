...
getConfigParamFromFile() {
    ...
    php -r "
        ...
        if (file_exists('/var/www/html/data/$TENANT/config-internal.php')) {
            \$config=include('/var/www/html/data/$TENANT/config-internal.php');
        ...
        if (file_exists('/var/www/html/data/$TENANT/config.php')) {
            \$config=include('/var/www/html/data/$TENANT/config.php');
...
start() {
    if [ -f "/var/www/html/data/$TENANT/config.php" ]; then
        local isInstalled=$(getConfigParamFromFile "isInstalled")
...
actionReinstall() {
    ...
    rm -rf /var/www/html/data/$TENANT/cache
...
# Export the variable so it can be used by other scripts or processes
export TENANT
...
# Ensure your custom script runs after the default entrypoint script finishes
/scripts/nurdscrm.sh
